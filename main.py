...
def procesar_datos():
    '''Procesa los tramos y prepara las solicitudes para su visualizacion.'''
...
def mostrar_siguiente():
    '''Muestra la siguiente solicitud de carga y su ruta optima.'''
    ...
    if not resultado.startswith("No rutas disponibles"):
        graficar_itinerario_por_carga(tramos_guardados[id_actual], id_actual)

    indice_actual += 1

def main():
    """Configura y lanza la interfaz grafica."""
    # Interfaz
    app = tk.Tk()
    app.title("Gestion de Envios")
